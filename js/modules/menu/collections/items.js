define([
	...
	function($, _, Backbone, MenuItem) {
		var allowed = new Set([
			'books', 'questions', 'favorites', 'comments', 'add'
		])
	    ...
	    var Collection = Backbone.Collection.extend({
	        ...
	        initialize: function(models, options) {
	        	...
	        	this.listenTo(Backbone, 'menu:additional', function(params) {
	        		...
	        		for (var i = 0; i < that.models.length; ++i) {
	        			if (that.models[i].id === 101) {
	        				that.add(new MenuItem({name: 'add', title: '"' + params.model.get('title') + '"', path: params.path}), {merge: true}, {at: i})
	        			}
	        		}
	        		that.add(new MenuItem({name: 'add', title: '"' + params.model.get('title') + '"', path: params.path}), {merge: true})
	        		that.trigger('add')
	        	...
	        	this.listenTo(Backbone, 'signin:success', function(options) {
	        		...
	        		that.add(new MenuItem({name: 'favorites'}), {merge: true})
	        	})

	        	this.listenTo(Backbone, 'signout', function(options) {
	        		if (this.menu != 'header') return

	        		that.remove(new MenuItem({name: 'favorites'}), {merge: true})
	        	})
	        }
	    })
